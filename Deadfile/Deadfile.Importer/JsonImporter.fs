...
type JsonImporter(repository:IDeadfileRepository) =
    let cache = IdCache()
    let tryDo (f:DeadfileContext -> unit) =
        if (not (String.IsNullOrEmpty repository.ConnectionString)) then
            use context = new DeadfileContext(repository.ConnectionString)
            f context
        else ()
